...
```{r echo=FALSE, fig.keep='all', out.width="100%"}
knitr::include_graphics("pictures/piaget_schema.png")
```
...
```{r echo=FALSE, fig.keep='all', out.width="100%"}
knitr::include_graphics("pictures/assimilation.png")
```
...
```{r echo=FALSE, fig.keep='all', out.width="100%"}
knitr::include_graphics("pictures/akkomodation.png")
```

\
"Kann eine Situation nicht durch bestehende Schemata erfolgreich bewältigt werden [Assimilation], so muss das entsprechende Schema um
die neuen Erkenntnisse erweitert werden [Akkommodation]."

```{r echo=FALSE, fig.keep='all', out.width="100%"}
knitr::include_graphics("pictures/peanuts_assimilation_akkomodation.png")
```

"In diesem Beispiel versucht Linus zunächst zu assimilieren: Er versucht mit dem Keks so umzugehen, wie er es mit Brot gewöhnt ist: Eine
Scheibe Brot kann man biegen. Nach einigen fehlgeschlagenen Versuchen
akkommodiert er: Ein Keks kann nicht mit Brot gleichgestellt werden.
Es handelt sich zwar bei beiden um etwas Essbares und um eine Backware, dennoch gibt es Unterschiede. Ein Keks ist etwas anderes, als eine
Scheibe Brot - das vorhandene Schema muss erweitert werden (Akkommodation), da es nicht ausreicht." <http://www.lern-psychologie.de/kognitiv/Piaget1.pdf>
